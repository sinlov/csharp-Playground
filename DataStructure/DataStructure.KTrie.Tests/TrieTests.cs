...
namespace DataStructure.KTrie.Tests
{
    public class TrieTests : BaseTests
    {
        ...
        public void GetEnumerator()
        {
            ...
            var resultEnumerator =
                trie.OfType<KeyValuePair<IEnumerable<char>, bool>>().Select(k => new string(k.Key.ToArray()))
                    .OrderBy(w => w).ToArray();
        ...
        public void KeysValues()
        {
            ...
            Assert.True(
                new[] { "AB", "ABC", "ABCDE", "ADE" }.SequenceEqual(trie.Keys.Select(i => new string(i.ToArray()))
                    .OrderBy(s => s)));
        ...
        public void Remove()
        {
            ...
            Assert.False(
                ((IDictionary<IEnumerable<char>, bool>)trie).Remove(
                    new KeyValuePair<IEnumerable<char>, bool>("XY", true)));
            Assert.False(
                ((IDictionary<IEnumerable<char>, bool>)trie).Remove(
                    new KeyValuePair<IEnumerable<char>, bool>("ABCD", true)));
            Assert.False(
                ((IDictionary<IEnumerable<char>, bool>)trie).Remove(
                    new KeyValuePair<IEnumerable<char>, bool>("ABCDE", true)));
            Assert.Equal(initialCount, trie.Count);
            Assert.True(
                ((IDictionary<IEnumerable<char>, bool>)trie).Remove(
                    new KeyValuePair<IEnumerable<char>, bool>("ABCDE", false)));
        ...
        public TrieTests(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
