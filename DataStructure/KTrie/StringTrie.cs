...
namespace DataStructure.KTrie
{
    ...
    public class StringTrie<TValue> : IDictionary<string, TValue>
    {
        ...
        bool ICollection<KeyValuePair<string, TValue>>.Contains(KeyValuePair<string, TValue> item) =>
            ((IDictionary<IEnumerable<char>, TValue>)_trie).Contains(
                new KeyValuePair<IEnumerable<char>, TValue>(item.Key, item.Value));

        public void CopyTo(KeyValuePair<string, TValue>[] array, int arrayIndex) =>
            Array.Copy(
                _trie.Select(i => new KeyValuePair<string, TValue>(new string(i.Key.ToArray()), i.Value)).ToArray(), 0,
                array, arrayIndex, Count);

        IEnumerator IEnumerable.GetEnumerator() => GetEnumerator();

        bool ICollection<KeyValuePair<string, TValue>>.Remove(KeyValuePair<string, TValue> item) =>
            ((IDictionary<IEnumerable<char>, TValue>)_trie).Remove(
                new KeyValuePair<IEnumerable<char>, TValue>(item.Key, item.Value));
    }
}